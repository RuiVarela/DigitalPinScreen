...
### 1 - Introdução
~~~
Dentro do directório VR_Pinscreen encontra-se a seguinte estrutura de directórios:
\VR_Pinscreen_-_src				Código Fonte da aplicação
\VR_Pinscreen_-_bin				Aplicação compilada (necessita da instalação prévida do openscenegraph)
\VR_Pinscreen_-_bin_with_osg	Bundle da aplicação compilada com as bibliotecas do openscenegraph. Aplicação pronta para uso.
~~~

### 2 - Estrutura de dados da aplicação
~~~
...
Para criar animações apartir de videos foi usada a aplicação livre virtualdub - http://www.virtualdub.org/. Para auxilixar a criação dos ficheiros de animação foram criados alguns scripts que estão no ficheiro helper_scripts.txt no directório de código fonte.
